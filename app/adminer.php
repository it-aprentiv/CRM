...
{
	return
		substr(preg_replace("~(?<=[?&])($Sf" . (SID ? "" : "|" . session_name()) . ")=[^&]*&~", '', "$_SERVER[REQUEST_URI]&"), 0, -1);
}
function
pagination($D, $Nb)
{
...
get_file($y, $Vb = false)
{
	...
	foreach ($Xc["error"] as $y => $n) {
		...
		if ($Vb) {
			$Ih = substr($Bb, 0, 3);
			if (function_exists("iconv") && preg_match("~^\xFE\xFF|^\xFF\xFE~", $Ih, $Mg)) $Bb = iconv("utf-16", "utf-8", $Bb);
			elseif ($Ih == "\xEF\xBB\xBF") $Bb = substr($Bb, 3);
			$H .= $Bb . "\n\n";
		} else $H .= $Bb;
	}
	return $H;
...
input($o, $Y, $r)
{
	...
	h($od[""]) . "<td>" . $b->editInput($_GET["edit"], $o, $Ka, $Y);
		...
		if ($Wd != "") echo $Wd;
		elseif (preg_match('~bool~', $o["type"])) echo "<input type='hidden'$Ka value='0'>" . "<input type='checkbox'" . (preg_match('~^(1|t|true|y|yes|on)$~i', $Y) ? " checked='checked'" : "") . "$Ka value='1'>";
		elseif ($o["type"] == "set") {
			preg_match_all("~'((?:[^']|'')*)'~", $o["length"], $Ie);
			foreach ($Ie[1] as $s => $X) {
				$X = stripcslashes(str_replace("''", "'", $X));
		...
		elseif (($hi = preg_match('~text|lob|memo~i', $o["type"])) || preg_match("~\n~", $Y)) {
			if ($hi && $x != "sqlite") $Ka .= " cols='50' rows='12'";
			else {
...
select_value($X, $_, $o, $ii)
{
	...
	if ($H !== null) {
		if (!is_utf8($H)) $H = "\0";
		elseif ($ii != "" && is_shortable($o)) $H = shorten_utf8($H, max(0, +$ii));
		else $H = h($H);
	}
...
if (isset($_GET["sqlite"]) || isset($_GET["sqlite2"])) {
	...
	fields($Q)
	{
		...
		foreach (get_rows("PRAGMA table_info(" . table($Q) . ")") as $I) {
			...
			if ($I["pk"]) {
				if ($pg != "") $H[$pg]["auto_increment"] = false;
...
if (isset($_GET["simpledb"])) {
	...
	Min_SQL
	{
		...
		{
			$H = array();
			if (preg_match_all("~itemName\(\) = (('[^']*+')+)~", $Ag, $Ie)) $H = array_map('idf_unescape', $Ie[1]);
			else {
		...
		insert($Q, $N)
		{
			...
			foreach ($N
				...
				if ($Y != "NULL") {
					$B = idf_unescape($B);
					if ($B == "itemName()") $Tf["ItemName"] = idf_unescape($Y);
					else {
						foreach ((array)$Y
							as $X) {
...
if (isset($_GET["mongo"])) {
	...
	if (class_exists('MongoDB')) {
		...
		sql_query_where_parser($Ag)
		{
			...
			foreach ($pj
				as $nj) $Z[] = trim($nj);
			if (count($qj) == 1) $qj = array();
			elseif (count($qj) > 1) $Z = array();
			return
				where_to_query($Z, $qj);
		}
		function
			...
			foreach (array('and' => $lj, 'or' => $mj) as $T => $Z) {
				if (is_array($Z)) {
					foreach ($Z
						...
						if ($T == 'and') $Pb['$and'][] = array($ob => array($yf => $X));
						elseif ($T == 'or') $Pb['$or'][] = array($ob => array($yf => $X));
					}
				}
			}
			return $Pb;
	...
	alter_indexes($Q, $c)
	{
		...
		foreach ($c
			as $X) {
			list($T, $B, $N) = $X;
			if ($N == "DROP") $H = $h->_db->command(array("deleteIndexes" => $Q, "index" => $B));
			else {
				$f = array();
				foreach ($N
					as $e) {
	...
	Min_SQL
	{
		...
		select($Q, $K, $Z, $rd, $Ef = array(), $z = 1, $D = 0, $rg = false)
		{
			...
			foreach ($Z
				...
				if ($ob == "_id") $Pb["query"]["ids"]["values"][] = $X;
				elseif ($ob . $X != "") {
					$fi = array("term" => array(($ob != "" ? $ob : "_all") => $X));
					if ($yf == "=") $Pb["query"]["filtered"]["filter"]["and"][] = $fi;
					else $Pb["query"]["filtered"]["query"]["bool"]["must"][] = $fi;
				}
			}
...
if (!defined("DRIVER")) {
	...
	Min_SQL
	{
		...
		slowQuery($F, $ki)
		{
			if (min_version('5.7.8', '10.1.2')) {
				if (preg_match('~MariaDB~', $this->_conn->server_info)) return "SET STATEMENT max_statement_time=$ki FOR $F";
				elseif (preg_match('~^(SELECT\b)(.+)~is', $F, $A)) return "$A[1] /*+ MAX_EXECUTION_TIME(" . ($ki * 1000) . ") */ $A[2]";
			}
		}
		function
		convertSearch($u, $X, $o)
	...
	db_collation($l, $qb)
	{
		...
		if (preg_match('~ COLLATE ([^ ]+)~', $Hb, $A)) $H = $A[1];
		elseif (preg_match('~ CHARACTER SET ([^ ]+)~', $Hb, $A)) $H = $qb[$A[1]][-1];
		return $H;
	}
	function
	engines()
...
Adminer
{
	...
	selectLinks($Th, $N = "")
	{
		...
		if (support("table")) {
			if (is_view($Th)) $Ae["view"] = lang(44);
			else $Ae["create"] = lang(45);
		}
	...
	selectSearchProcess($p, $w)
	{
		...
		foreach ((array)$_GET["where"] as $y => $X) {
			if ("$X[col]$X[val]" != "" && in_array($X["op"], $this->operators)) {
				...
				if ($X["col"] != "") $H[] = $ng . $m->convertSearch(idf_escape($X["col"]), $X, $p[$X["col"]]) . $yb;
				else {
					$sb = array();
					foreach ($p
						as $B => $o) {
			...
			if ($G) {
				...
				while ($I = $G->$Uc()) {
					...
					if ($_POST["format"] != "sql") {
						...
						$dh = ($Ke ? "\n" : " ") . "(" . implode(",\t", $I) . ")";
						if (!$Za) $Za = $Xd . $dh;
						elseif (strlen($Za) + 4 + strlen($dh) + strlen($Ph) < $Ke) $Za .= ",$dh";
						else {
							echo $Za . $Ph;
							$Za = $Xd . $dh;
						}
...
', script("setupSubmitHighlight(document);");
					...
					auth_error($n)
					{
						...
						if (isset($_GET["username"])) {
							...
							if (($_COOKIE[$sh] || $_GET[$sh]) && !$zd) $n = lang(87);
							else {
								restart_session();
								add_invalid_login();
								$E = get_password();
						...
						for ($s = 0; (!$z || $s < $z) && ($I = $G->fetch_row()); $s++) {
							if (!$s) {
								...
								for ($ge = 0; $ge < count($I); $ge++) {
									...
									$Ff = $o->orgname;
									$H[$o->table] = $Gf;
									if ($Hf && $x == "sql") $Ae[$ge] = ($B == "table" ? "table=" : ($B == "possible_keys" ? "indexes=" : null));
									elseif ($Gf != "") {
										if (!isset($w[$Gf])) {
											$w[$Gf] = array();
							...
							foreach ($I
								...
								elseif ($Va[$y] && !is_utf8($X)) $X = "<i>" . lang(48, strlen($X)) . "</i>";
								else {
									$X = h($X);
...
<td><input type="radio" name="auto_increment_col" value=""><acronym id="label-ai" title="', lang(52), '">AI</acronym>', doc_link(array('sql' => "example-auto-increment.html", 'mariadb' => "auto_increment/", 'sqlite' => "autoinc.html", 'pgsql' => "datatype.html#DATATYPE-SERIAL", 'mssql' => "ms186775.aspx",)), '<td id="label-default"', $Xb, '>', lang(53), (support("comment") ? "<td id='label-comment'$wb>" . lang(51) : "");
						...
						echo '<td>', "<input type='image' class='icon' name='add[" . (support("move_col") ? 0 : count($p)) . "]' src='" . h(preg_replace("~\\?.*~", "", ME) . "?file=plus.gif&version=4.7.6") . "' alt='+' title='" . lang(108) . "'>" . script("row_count = " . count($p) . ";"), '</thead>
<tbody>
...
', ($T == "PROCEDURE" ? "<td>" . html_select("fields[$s][inout]", explode("|", $Vd), $o["inout"]) : ""), '<th>';
					...
					{
						if ($_POST["drop"]) query_redirect($jc, $Be, $Te);
						elseif ($rf == "") query_redirect($Hb, $Be, $Se);
						elseif ($rf != $ef) {
							$Jb = queries($Hb);
					...
					if (isset($_GET["download"])) {
						...
						foreach (table_status('', true) as $Q => $R) {
							...
							foreach ($b->foreignKeys($Q) as $X) {
								if (!$X["db"]) {
									$ue = $Sa;
									if ($Wh[$Q][1] || $Wh[$X["table"]][1]) $ue = min(floatval($Wh[$Q][1]), floatval($Wh[$X["table"]][1])) - 1;
									else $Sa -= .1;
						...
						foreach ($fh
							...
							foreach ((array)$Kg[$B] as $di => $Lg) {
								foreach ($Lg
									...
									$s = 0;
									foreach ($f
										as $ci) echo "\n<div class='references' title='" . h($di) . "' id='refd$ue-" . ($s++) . "' style='left: $ve" . "em; top: " . $Q["fields"][$ci]["pos"] . "em; height: 1.25em; background: url(" . h(preg_replace("~\\?.*~", "", ME) . "?file=arrow.gif) no-repeat right center;&version=4.7.6") . "'><div style='height: .5em; border-bottom: 1px solid Gray; width: " . (-$ve) . "em;'></div></div>";
								}
							}
							echo "\n</div>\n";
						}
							...
							if (is_string($F)) {
								...
								while ($F != "") {
									if (!$C && preg_match("~^$Bh*+DELIMITER\\s+(\\S+)~i", $F, $A)) {
										...
										list($kd, $jg) = $A[0];
										if (!$kd && $md && !feof($md)) $F .= fread($md, 1e5);
										else {
											if (!$kd && rtrim($F) == "") break;
											$C = $jg + strlen($kd);
											if ($kd && rtrim($kd) != $ac) {
												while (preg_match('(' . ($kd == '/*' ? '\*/' : ($kd == '[' ? ']' : (preg_match('~^-- |^#~', $kd) ? "\n" : preg_quote($kd) . "|\\\\."))) . '|$)s', $F, $A, PREG_OFFSET_CAPTURE, $C)) {
													$dh = $A[0][0];
													if (!$dh && $md && !feof($md)) $F .= fread($md, 1e5);
													else {
														$C = $A[0][1] + strlen($dh);
														if ($dh[0] != "\\") break;
													}
...
';
						...
						if (!isset($_GET["import"])) {
							$zg = $_GET["sql"];
							if ($_POST) $zg = $_POST["query"];
							elseif ($_GET["history"] == "all") $zg = $Bd;
...
';
						...
						if ($_POST && !$n && !isset($_GET["select"])) {
							$Be = $_POST["referer"];
							if ($_POST["insert"]) $Be = ($Oi ? null : $_SERVER["REQUEST_URI"]);
							elseif (!preg_match('~^.+&select=.+$~', $Be)) $Be = ME . "select=" . urlencode($a);
							$w = indexes($a);
							$Ji = unique_array($_GET["where"], $w);
							$Bg = "\nWHERE $Z";
							if (isset($_POST["delete"])) queries_redirect($Be, lang(166), $m->delete($a, $Bg, !$Ji));
							else {
								$N = array();
								foreach ($p
									as $B => $o) {
...
';
						...
						foreach ($I["indexes"] as $v) {
							if (!$_POST["drop_col"] || $ge != key($_POST["drop_col"])) {
								...
								echo "<td><input name='indexes[$ge][name]' value='" . h($v["name"]) . "' autocapitalize='off' aria-labelledby='label-name'>\n", "<td><input type='image' class='icon' name='drop_col[$ge]' src='" . h(preg_replace("~\\?.*~", "", ME) . "?file=cross.gif&version=4.7.6") . "' alt='x' title='" . lang(111) . "'>" . script("qsl('input').onclick = partial(editingRemoveRow, 'indexes\$1[type]');");
							}
							$ge++;
						}
						echo '</table>
...
';
						...
						$B = DB;
						if ($_POST) $B = $I["name"];
						elseif (DB != "") $I["collation"] = db_collation(DB, $qb);
						elseif ($x == "sql") {
...
';
						...
						if ($_POST && !$n) {
							...
							if ($_POST["drop"]) query_redirect("DROP SCHEMA " . idf_escape($_GET["ns"]), $_, lang(188));
							else {
								$B = trim($I["name"]);
								$_ .= urlencode($B);
								if ($_GET["ns"] == "") query_redirect("CREATE SCHEMA " . idf_escape($B), $_, lang(189));
								elseif ($_GET["ns"] != $B) query_redirect("ALTER SCHEMA " . idf_escape($_GET["ns"]) . " RENAME TO " . idf_escape($B), $_, lang(190));
								else
									redirect($_);
							}
						}
...
';
						...
						if ($_POST && !$n && !$_POST["add"] && !$_POST["change"] && !$_POST["change-js"]) {
							...
							if ($x == "sqlite") queries_redirect($Be, $Qe, recreate_table($a, $a, array(), array(), array(" $B" => ($_POST["drop"] ? "" : " " . format_foreign_key($I)))));
							else {
								$c = "ALTER TABLE " . table($a);
								$jc = "\nDROP " . ($x == "sql" ? "FOREIGN KEY " : "CONSTRAINT ") . idf_escape($B);
								if ($_POST["drop"]) query_redirect($c . $jc, $Be, $Qe);
								else {
...
';
						...
						if ($_POST && !$n) {
							...
							if (!$_POST["drop"] && $a == $B && $x != "sqlite" && $T == "VIEW" && $Lf == "VIEW") query_redirect(($x == "mssql" ? "ALTER" : "CREATE OR REPLACE") . " VIEW " . table($B) . $Ha, $Be, $Qe);
							else {
...
';
						...
						if ($_POST && !$n) {
							$_ = substr(ME, 0, -1);
							$B = trim($I["name"]);
							if ($_POST["drop"]) query_redirect("DROP SEQUENCE " . idf_escape($fa), $_, lang(222));
							elseif ($fa == "") query_redirect("CREATE SEQUENCE " . idf_escape($B), $_, lang(223));
...
';
						...
						if ($_POST && !$n) {
							$_ = substr(ME, 0, -1);
							if ($_POST["drop"]) query_redirect("DROP TYPE " . idf_escape($ga), $_, lang(227));
							else
...
';
						...
						if ($_POST) {
							if (!$n && in_array($_POST["Timing"], $Di["Timing"]) && in_array($_POST["Event"], $Di["Event"]) && in_array($_POST["Type"], $Di["Type"])) {
								...
								$Be = ME . "table=" . urlencode($a);
								if ($_POST["drop"]) query_redirect($jc, $Be, lang(231));
								else {
									if ($B != "") queries($jc);
...
';
						...
						if ($_POST && !$n) {
							...
							if ($_POST["drop"]) query_redirect("DROP USER $tf", ME . "privileges=", lang(238));
							else {
								...
								if (!$n) {
									...
									foreach ($df
										...
										$pd = array_keys($pd);
										if (isset($_GET["grant"])) $Wg = array_diff(array_keys(array_filter($df[$nf], 'strlen')), $pd);
										elseif ($tf == $ff) {
						...
						foreach (array("" => "", "Server Admin" => lang(35), "Databases" => lang(38), "Tables" => lang(131), "Columns" => lang(49), "Procedures" => lang(242),) as $Db => $bc) {
							foreach ((array)$ug[$Db] as $tg => $vb) {
								...
								foreach ($qd
									...
									$Y = $pd[strtoupper($tg)];
									if ($Db == "Server Admin" && $nf != (isset($qd["*.*"]) ? "*.*" : ".*")) echo "<td>";
									elseif (isset($_GET["grant"])) echo "<td><select name=$B><option><option value='1'" . ($Y ? " selected" : "") . ">" . lang(243) . "<option value='0'" . ($Y == "0" ? " selected" : "") . ">" . lang(244) . "</select>";
									else {
...
', script("tableCheck();");
						...
						if ($_POST && !$n) {
							...
							if ($_POST["export"]) {
								...
								$td = ($rd && $ce ? "\nGROUP BY " . implode(", ", $rd) : "") . ($Ef ? "\nORDER BY " . implode(", ", $Ef) : "");
								if (!is_array($_POST["check"]) || $pg) $F = "SELECT $nd$oj$td";
								else {
									$Ii = array();
							...
							if (!$G) echo "<p class='error'>" . error() . "\n";
								...
								if (!is_ajax()) {
									if ($J || $D) {
										$Fc = true;
										if ($_GET["page"] != "last") {
											if ($z == "" || (count($J) < $z && ($J || !$D))) $ld = ($D ? $D * $z : 0) + count($J);
											elseif ($x != "sql" || !$ce) {
												$ld = ($ce ? false : found_rows($R, $Z));
												if ($ld < max(1e4, 2 * ($D + 1) * $z)) $ld = reset(slow_query(count_rows($a, $Z, $ce, $rd)));
												else $Fc = false;
											}
										}
